...
QALIGN_LEFT = QtCore.Qt.AlignLeft
QALIGN_CENTER = QtCore.Qt.AlignCenter
QALIGN_RIGHT = QtCore.Qt.AlignRight
QALIGN_TOP = QtCore.Qt.AlignTop
...
class QConsole(QtWidgets.QPlainTextEdit):
	def __init__(self, parent, *args, **kwargs):
		...
		self.args = args
		self.kwargs = kwargs
		self.consoleState = Generic_class(scrollBottom = True)
...
def makeWidget(widgetClass, layoutDirection="vertical", parent=None):
	...
	layout.setAlignment(QALIGN_TOP | QALIGN_LEFT)
...
def makeLabel(s, y, a=QALIGN_CENTER):
...
class QSimpleTable(QtWidgets.QTableWidget):
	...
	def setHeaders(self, *headers):
		...
		for i, header in enumerate(headers):
			...
			item.setTextAlignment(QALIGN_CENTER)
			item.setFont(font)
	...
	def insertStuff(self, row, col, text=None, icon=None, rowSpan=1, colSpan=1, alignment=None, font=None, resize=True):
		...
		alignment = alignment if alignment else QALIGN_CENTER
